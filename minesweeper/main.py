...
def createGameWindow(rowX, columnY,numberOfBombs):
    ...
    for row in range(rowX):
        for column in range(columnY):
            frameToCreate = cellFrame(row, column, gridFrame, gameWindow,edgeDict,columnY,numberOfBombs)
...
# --------------------------------------------------------- Root window ------------------------------------------------
root = tk.Tk()
newGameButton = tk.Button(text="New game", command=lambda: createGameWindow(16, 30, 40))   # (x,y,bombs)
newGameButton.pack()
